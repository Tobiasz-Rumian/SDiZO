...
public class Table implements Structure {
   ...
   public void subtract(Place place, int number) {
      if (table == null || table.length == 0) {
         return;//Jeżeli tablica nie zawiera rekordów, nic nie rób
      }
      ...
      switch (place) {
         ...
         case END:
            ...
            break;
         case RANDOM:
            int random = Selector.randomIntegerFromRange(0, table.length);//Generowanie losowego indeksu.
   ...
   public void add(Place place, int number) {
      ...
      switch (place) {
         ...
         case END:
            for (int i = 0; i < table.length; i++) {
               x[i] = table[i];//Przekopiowywanie tablicy
            }
            x[x.length - 1] = number;//Wstawianie nowej liczby na ostatnie miejsce
            break;
         case RANDOM:
            int random = Selector.randomIntegerFromRange(0, table.length); //Losowanie indeksu
            for (int i = 0; i < random; i++) {
               x[i] = table[i];//Przekopiowywanie tablicy od indeksu 0 do wylosowanego indeksu-1
            }
   ...
   public void subtract(int index) {
      if (table != null || index <= table.length - 1) {
         if (table.length == 1) {
