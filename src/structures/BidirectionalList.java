...
public class BidirectionalList implements Structure {
	...
	public void subtract(Place place, int number) {
		if (firstElement.getRight() == null || lastElement.getLeft() == null) {
			clear();
		}
		if (size == 0) {
			return;
	...
	public void add(Place place, int number) {
		...
		switch (place) {
		case START: {
			Node newNode = new Node(firstElement, number,
				PlaceOnList.RIGHT);//Tworzenie nowego elementu z prawym dzieckiem jako pierwszym elementem tablicy
		...
		case END: {
			Node newNode = new Node(lastElement, number,
				PlaceOnList.LEFT);//Tworzenie nowego elementu z lewym dzieckiem jako ostatnim elementem tablicy
	...
	public boolean find(int find) {
		...
		for (int i = 0; i < size; i++) {
			if (node.getValue() == find) {
				return true;//Sprawdź, czy sprawdzany węzeł == szukany węzeł
			}
	...
	@Override
	public String show() {
		if (size == 0 || firstElement == null || lastElement == null) {
			return "";
		}
	...
	public void add(int index, int value) {
		if (firstElement != null && lastElement != null && index > size) {
			return;
		}
		if (firstElement == null || lastElement == null
			|| size == 0) {//Jeżeli rozmiar tablicy==0 Ustaw nowy węzeł jako pierwszy i ostatni element
			clear();
	...
	public void subtract(int value) {
		if (firstElement == null || lastElement == null) {
			clear();
		} else if (firstElement.getRight() == null || lastElement.getLeft() == null) {
			clear();
		} else {
			Node node = firstElement;
			for (int i = 0; i < size; i++) {
				if (node.getValue() == value) {
					break;//Sprawdź, czy sprawdzany węzeł == szukany węzeł
				}
				node = node.getRight();//Idź do prawego węzła
			}
			if (node == null) {
